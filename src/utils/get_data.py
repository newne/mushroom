...
class GetData(SendRequest):
    ...
    def _load_prompt_from_mlflow(self, prompt_uri: str):
        ...
        self._cached_prompt_expires_at = (
            time.time() + self._get_prompt_cache_ttl_seconds()
        )
    ...
    def get_mushroom_prompt(self):
        ...
        """
        if self._cached_prompt and time.time() < float(
            self._cached_prompt_expires_at or 0.0
        ):
            return self._cached_prompt

        try:

            def _fetch_prompt_from_api(api_url: Optional[str]) -> Optional[str]:
                if not api_url:
                    return None

                if not hasattr(settings, "prompt"):
                    logger.warning(
                        "[Prompt API] 配置文件中未找到prompt配置，使用默认提示词"
                    )
                    return None
                ...
                logger.info(f"[Prompt API] 正在从API获取提示词: {api_url}")
                ...
                if response.status_code != 200:
                    logger.error(
                        f"[Prompt API] API请求失败，状态码: {response.status_code}, 响应: {response.text}"
                    )
                    return None
                ...
                if "data" in data and isinstance(data["data"], dict):
                    ...
                    if "content" in data_obj and isinstance(data_obj["content"], dict):
                        ...
                        if "template" in content_obj and isinstance(
                            content_obj["template"], str
                        ):
                            prompt_content = content_obj["template"]
                            logger.debug(
                                "[Prompt API] 从data.content.template获取提示词"
                            )
                ...
                if not prompt_content:
                    logger.warning("[Prompt API] API响应中未找到提示词内容")
                ...
                self._cached_prompt_source = api_url
                self._cached_prompt_expires_at = (
                    time.time() + self._get_prompt_cache_ttl_seconds()
                )
                logger.info(
                    f"[Prompt API] 成功获取提示词，长度: {len(prompt_content)} 字符"
                )
            ...
            prompt_url_bak = None
            if hasattr(self.urls, "prompt_mushroom_description_bak"):
                prompt_url_bak = self.urls.prompt_mushroom_description_bak
            elif hasattr(self.urls, "PROMPT_MUSHROOM_DESCRIPTION_BAK"):
                prompt_url_bak = self.urls.PROMPT_MUSHROOM_DESCRIPTION_BAK

            if not prompt_url and not prompt_url_bak:
                logger.warning(
                    "[Prompt API] 配置文件中未找到prompt_mushroom_description，使用默认提示词"
                )
                fallback_prompt = getattr(
                    settings.llama, "mushroom_descripe_prompt", None
                )
            ...
            if prompt_url_bak:
                prompt_url_bak = prompt_url_bak.format(host=self.host)

            if prompt_url and str(prompt_url).startswith("prompts:/"):
                prompt_value = self._load_prompt_from_mlflow(str(prompt_url))
                if prompt_value is not None:
                    return prompt_value
                logger.warning("[Prompt MLflow] 获取失败，尝试使用备份API")
                prompt_value = _fetch_prompt_from_api(prompt_url_bak)
                if prompt_value is not None:
                    return prompt_value
                logger.warning("[Prompt MLflow] 备份API获取失败，使用默认提示词")
                fallback_prompt = getattr(
                    settings.llama, "mushroom_descripe_prompt", None
                )
                return fallback_prompt

            prompt_value = _fetch_prompt_from_api(prompt_url)
            if prompt_value is not None:
                return prompt_value
