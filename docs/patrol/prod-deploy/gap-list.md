...
| 项 | 结论 | 对代码的影响 |
| --- | --- | --- |
| **限位开关** | **Y、Z 两轴都装了独立限位开关**（现场确认） | `home_dir`（**两轴都是 2 = 负限位**，2026-09-15 更正）**有硬件依据**；控制器 `homeTime` 是有效保护；B4 关闭 |
...
| # | 信息项 | **实测结果** | 结论 |
| --- | --- | --- | --- |
| B1 ★ | 软限位 | 轴0 `[-200, 200]`、**轴1(Y) `[0, 4495]`**、**轴2(Z) 已取消**（原始 `softLimitMax = -1`） | Y ✅ 已整定，与行程 4492 一致；**Z 无控制器侧保护**。⚠️ **2026-09-15（ADR-0018）**：Z 的行程已改成 `0…212`，上机时应把 Z 的软限位整定为 `min=0 / max=212`（见 `z-frame-cutover.md` §3） |
...
| B6 | 回零方向 | ⚠️ **2026-09-15 重开又关闭**：当年（2026-09-13）只验到"控制器在自己找的开关处置零"，据此断言 Z 的 0 在顶端——**错了**，实际落在底部。现场指出后按说明书 §三.2（正限位=远离电机端）改成两轴都回**负限位**，Z 行程随之改为 `0…212`（向下为正）。见 ADR-0018 与 `z-frame-cutover.md` | 关闭（按新框架上机回零后复核） |
...
> **2026-09-15 更新（ADR-0018）**：Z 的框架已改成"顶端为 0、向下为正、行程 `0…212`"，
> 下面这段的 `-212…0` 是当时的值。结论不变——**Z 的控制器软限位仍是取消状态**，
> 迁移时应当一并写成 `0…212`（见 `z-frame-cutover.md` §3）。
