...
def _scorer_log(event: str, message: str, level: str = "INFO", **context):
    log_task_event(
        "VISION_OFFLINE_SCORERS",
        event,
        message,
        level=level,
        task_type="helper",
    ...
    for output in eval_df["prediction"]:
        try:
            ...
            if len(scores) <= 5:  # Print first few errors
                _scorer_log(
                    "VISION_OFFLINE_SCORER_INVALID_JSON_DEBUG",
                    "输出不是有效 JSON",
                    level="DEBUG",
...
score_json_format_metric = make_metric(
    eval_fn=score_json_format, greater_is_better=True, name="score_json_format"
)
