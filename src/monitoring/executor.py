...
class SetpointMonitoringTask(BaseTask):
    ...
    def execute_task(self) -> Dict[str, Any]:
        ...
        if not static_configs:
            logger.warning(f"[{self.task_name}] ⚠️ 静态配置表中没有找到测点配置，使用备用方案")
            return self._execute_fallback_monitoring()
        
        logger.info(f"[{self.task_name}] ✅ 从静态配置表获取到 {len(static_configs)} 个测点配置")
        
        # 2. 按库房分组配置
        configs_by_room = self._group_configs_by_room(static_configs)
        
        logger.info(f"[{self.task_name}] 📍 涉及 {len(configs_by_room)} 个库房: {list(configs_by_room.keys())}")
        ...
        for room_id, room_configs in configs_by_room.items():
            try:
                logger.info(f"[{self.task_name}] 🔍 处理库房 {room_id} ({len(room_configs)} 个测点)")
                ...
                if room_changes:
                    logger.info(f"[{self.task_name}] ✅ 库房 {room_id}: 检测到 {len(room_changes)} 个变更")
        ...
        stored_records = 0
        if all_changes:
            logger.info(f"[{self.task_name}] 💾 存储 {len(all_changes)} 条变更记录到数据库...")
            stored_records = self._store_setpoint_changes_to_database(all_changes)
            
            if stored_records == len(all_changes):
                logger.info(f"[{self.task_name}] ✅ 成功存储 {stored_records} 条变更记录")
            else:
                logger.warning(f"[{self.task_name}] ⚠️ 部分存储失败: {stored_records}/{len(all_changes)}")
        else:
            logger.info(f"[{self.task_name}] ℹ️ 无变更记录需要存储")
        
        processing_time = (datetime.now() - processing_start).total_seconds()
        
        logger.info(f"[{self.task_name}] 🎯 监控完成: {successful_rooms}/{len(configs_by_room)} 库房成功")
    ...
    def _group_configs_by_room(self, static_configs: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
        """
        按库房分组静态配置
        ...
        for room_id, room_configs in configs_by_room.items():
            ...
            logger.debug(f"[{self.task_name}] 库房 {room_id}: {len(room_configs)} 个测点, 设备类型: {device_types}")
    ...
    def _monitor_room_with_static_configs(
        ...
        try:
            ...
            # 1. 获取实时数据
            realtime_data = self._get_realtime_setpoint_data(room_id, room_configs, start_time, end_time)
            
            if realtime_data.empty:
                logger.debug(f"[{self.task_name}] 库房 {room_id} 无实时数据")
                return []
            
            logger.debug(f"[{self.task_name}] 库房 {room_id} 获取到 {len(realtime_data)} 条实时数据")
            
            # 2. 检测变更
            changes = self._detect_changes_with_static_configs(realtime_data, room_configs)
            
            logger.debug(f"[{self.task_name}] 库房 {room_id} 检测到 {len(changes)} 个变更")
    ...
    def _get_realtime_setpoint_data(
        ...
        try:
            ...
            logger.debug(f"[{self.task_name}] 库房 {room_id} 获取到 {len(df)} 条实时数据")
    ...
    def _detect_changes_with_static_configs(
        ...
        try:
            ...
            # 检查数据结构
            logger.debug(f"[{self.task_name}] 实时数据列: {list(realtime_data.columns)}")
            ...
            for group_key, group in grouped_data:
                ...
                if group_key_format == "device_alias_point_name":
                    ...
                    if not matching_config:
                        logger.debug(f"[{self.task_name}] 未找到匹配配置: {device_name}.{point_name}")
                        continue
                ...
                if not config:
                    logger.debug(f"[{self.task_name}] 未找到配置: {config_key}")
                    continue
    ...
    def _detect_point_changes(self, group: pd.DataFrame, config: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        检测单个测点的变更
        ...
        try:
            ...
            for i in range(1, len(group)):
                ...
                if change_detected:
                    ...
                    logger.debug(f"[{self.task_name}] {config['device_name']}.{config['point_name']}: {change_info.get('change_detail', '')}")
...
def safe_hourly_setpoint_monitoring() -> None:
    ...
    if not result.get('success', False):
        logger.error(f"[SETPOINT_MONITOR] 设定点监控任务失败: {result.get('error', '未知错误')}")
    else:
        logger.info(f"[SETPOINT_MONITOR] 设定点监控任务成功完成")
