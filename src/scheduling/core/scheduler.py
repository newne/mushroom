...
from ..tasks.job_registry import (
    register_jobs,
    log_registered_jobs,
...
class OptimizedScheduler:
    ...
    def _init_scheduler(self) -> BackgroundScheduler:
        ...
        try:
            ...
            scheduler.add_listener(
                exception_listener, events.EVENT_JOB_ERROR | events.EVENT_JOB_EXECUTED
            )
    ...
    def _register_signal_handlers(self) -> None:
        ...
        try:
            ...
            logger.warning(
                "[SCHEDULER] 无法在非主线程中注册信号处理器 (如在 Streamlit/Uvicorn 工作线程中运行)，跳过信号注册"
            )
        ...
        while not self.is_shutting_down:
            try:
                if not self.scheduler.running:
                    ...
                    logger.error(
                        f"[SCHEDULER] 调度器停止 (失败: {self.consecutive_failures}/{self.max_failures})"
                    )
    ...
    def initialize(self) -> None:
        ...
        for init_attempt in range(1, max_init_retries + 1):
            try:
                ...
                error_msg = str(e)
                logger.error(
                    f"[SCHEDULER] 初始化失败 (尝试 {init_attempt}/{max_init_retries}): {error_msg}"
                )
                ...
                if init_attempt < max_init_retries:
                    ...
                    logger.critical(
                        f"[SCHEDULER] 初始化失败，已达到最大重试次数 ({max_init_retries})"
                    )
                    raise
