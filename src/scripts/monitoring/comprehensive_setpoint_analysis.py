...
class SetpointAnalysisManager:
    ...
    def step2_execute_batch_analysis(self, start_time, end_time, description=""):
        ...
        print(f"分析时间范围: {start_time.strftime('%Y-%m-%d %H:%M:%S')} ~ {end_time.strftime('%Y-%m-%d %H:%M:%S')}")
                ...
                if row:
                    ...
                    if expected_changes is not None:
                        if row[0] == expected_changes:
                            print(f"✅ 数据一致性验证通过: 存储记录数({row[0]})与预期({expected_changes})一致")
                        else:
                            print(f"⚠️ 数据一致性异常: 存储记录数({row[0]})与预期({expected_changes})不一致")
                    ...
                    result = conn.execute(text("""
                        SELECT 
                            ...
                            COUNT(CASE WHEN change_magnitude = 0 THEN 1 END) as zero_magnitude,
                            COUNT(CASE WHEN change_detail = '' THEN 1 END) as empty_detail
                        FROM device_setpoint_changes 
                        WHERE detection_time >= :start_time
                    """), {"start_time": self.analysis_start_time})
    ...
    def step4_check_system_status(self):
        ...
        try:
            ...
            if self.analysis_results:
                ...
                # 检查是否有错误
                error_count = sum(len(result['error_rooms']) for result in self.analysis_results)
                if error_count == 0:
                    print("✅ 无错误记录")
                else:
    ...
    def step5_expand_analysis_timerange(self, target_date):
        ...
        while current_end.date() > target_date:
            batch_start = max(
                current_end - timedelta(days=batch_days),
                target_datetime
            )
        ...
        for i, (start, end) in enumerate(batches, 1):
            days = (end - start).days
            print(f"   批次 {i}: {start.strftime('%Y-%m-%d')} ~ {end.strftime('%Y-%m-%d')} ({days}天)")
    ...
    def generate_final_report(self):
        ...
        total_analysis_time = (datetime.now() - self.analysis_start_time).total_seconds()
        ...
        # 时间范围统计
        if self.analysis_results:
            earliest_start = min(result['start_time'] for result in self.analysis_results)
        ...
        # 性能统计
        total_processing_time = sum(result['processing_time'] for result in self.analysis_results)
        ...
        # 数据质量统计
        storage_rate = (self.total_records_stored / self.total_changes_detected * 100) if self.total_changes_detected > 0 else 0
        print(f"\\n📈 数据质量:")
