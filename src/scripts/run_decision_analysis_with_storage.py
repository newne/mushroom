...
def run_enhanced_decision_analysis(
    ...
    try:
        ...
        # 如果无法导入，尝试直接调用脚本
        return run_decision_analysis_via_script(
            room_id, analysis_datetime, output_format, verbose
        )
...
def run_decision_analysis_via_script(
    room_id: str,
    analysis_datetime: datetime,
    output_format: str = "both",
    verbose: bool = False,
    ...
    try:
        ...
        # 构建命令
        script_path = (
            Path(__file__).parent / "analysis" / "run_enhanced_decision_analysis.py"
        )
        ...
        # 查找匹配的文件（可能时间戳略有差异）
        output_files = list(
            output_dir.glob(f"enhanced_decision_analysis_{room_id}_*.json")
        )
...
def store_results_to_dynamic_table(
    json_data: Dict[str, Any], room_id: str, analysis_datetime: datetime
) -> Dict[str, Any]:
    """
    将分析结果存储到动态结果表
    ...
    try:
        ...
        storage_result = store_decision_analysis_results(
            json_data=json_data, room_id=room_id, analysis_time=analysis_datetime
        )

        logger.info("存储完成，统计信息:")
        logger.info(f"  批次ID: {storage_result.get('batch_id')}")
        logger.info(
            f"  动态结果记录数: {storage_result.get('dynamic_results_count', 0)}"
        )
...
def verify_storage_results(batch_id: str, room_id: str) -> bool:
    ...
    try:
        ...
        dynamic_results = query_decision_analysis_dynamic_results(
            room_id=room_id, batch_id=batch_id, limit=100
        )
...
def main():
    ...
    parser.add_argument(
        "--skip-analysis", action="store_true", help="跳过分析，直接使用最新的输出文件"
    )
    ...
    try:
        ...
        if args.skip_analysis:
            ...
            output_dir = Path(__file__).parent.parent / "output"
            output_files = list(
                output_dir.glob(f"enhanced_decision_analysis_{args.room_id}_*.json")
            )
