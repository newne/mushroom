...
def safe_daily_top_quality_clip_inference() -> None:
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            logger.info(
                f"[TOP_QUALITY_TASK] 开始执行Top质量图像任务 (日期 {target_date}, 尝试 {attempt}/{max_retries})"
            )
            ...
            error_msg = str(e)
            logger.error(
                f"[TOP_QUALITY_TASK] 任务失败 (尝试 {attempt}/{max_retries}): {error_msg}"
            )
            if _is_connection_error(error_msg) and attempt < max_retries:
                logger.warning(
                    f"[TOP_QUALITY_TASK] 检测到连接类错误，将在 {retry_delay} 秒后重试..."
                )
                time.sleep(retry_delay)
            else:
