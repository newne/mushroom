    ...
    try:
        ...
        for device_type, type_changes in device_types.items():
            ...
            for change in type_changes:
                ...
                print(f"     幅度: {change['change_magnitude']:.2f}")
                print()
...
def monitor_all_rooms(hours_back: int = 1, store_results: bool = True):
    ...
    try:
        ...
        for room_id, changes in all_changes.items():
            ...
            # 显示最近的几个变更
            recent_changes = sorted(changes, key=lambda x: x['change_time'], reverse=True)[:3]
            print("   最近变更:")
            for change in recent_changes:
                change_time = change['change_time'].strftime('%H:%M:%S')
                print(f"     - {change['device_name']}.{change['point_name']}: {change['change_detail']} ({change_time})")
            print()
    ...
    try:
        ...
        for device_type, configs in device_configs.items():
            ...
            for config in configs:
                ...
                print(f"   • {config.point_alias} ({config.change_type.value}{threshold_info}{enum_info})")
        ...
        try:
            ...
            device_types_in_config = [key for key in datapoint_config.keys() if key != 'remark']
...
使用示例:
  ...
  python scripts/monitor_setpoint_changes.py --create-table
    ...
    parser.add_argument('--hours', type=int, default=1, help='往前查询的小时数 (默认: 1)')
