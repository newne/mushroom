...
class DataExtractor:
    ...
    def extract_current_embedding_data(
        ...
        try:
            ...
            aggregation_start = target_datetime - timedelta(
                minutes=image_aggregation_window_minutes
            )
            aggregation_end = target_datetime + timedelta(
                minutes=image_aggregation_window_minutes
            )
            ...
            with Session(self.db_engine) as session:
                ...
                latest_quality = (
                    select(
                        ...
                        ImageTextQuality.image_quality_score.label(
                            "image_quality_score"
                        ),
                    )
                    .join(
                        ...
                        and_(
                            ...
                            == latest_text_quality.c.image_path,
                            ImageTextQuality.created_at
                            == latest_text_quality.c.max_created_at,
                        ),
                    )
                    .subquery()
                )
                ...
                df = self._add_multi_image_metadata(
                    df, aggregation_start, aggregation_end
                )
            ...
            logger.error(
                f"[DataExtractor] Failed to extract embedding data: {e}", exc_info=True
            )
            return pd.DataFrame()

    def extract_best_quality_embedding_today(
        self,
        room_id: str,
        target_datetime: datetime,
    ) -> pd.DataFrame:
        """
        Extract the highest-quality image embedding for a room between today's 00:00 and target time.
        ...
        Returns:
            DataFrame with a single best-quality embedding record (if available)
        """
        logger.info(
            f"[DataExtractor] Extracting best-quality embedding for today: "
            f"room_id={room_id}, datetime={target_datetime}"
        )
        ...
        try:
            ...
            day_start = target_datetime.replace(
            ...
            with Session(self.db_engine) as session:
                ...
                query = (
                    ...
                    .where(
                        and_(
                            MushroomImageEmbedding.room_id == room_id,
                            MushroomImageEmbedding.collection_datetime >= day_start,
                            MushroomImageEmbedding.collection_datetime
                            <= target_datetime,
                    ...
                    .order_by(
                        latest_quality.c.image_quality_score.desc().nullslast(),
                        MushroomImageEmbedding.collection_datetime.desc(),
                    )
                    .limit(1)
                )
                ...
                if not row:
                    logger.warning(
                        f"[DataExtractor] No quality-scored embedding found for room_id={room_id} "
                        f"between {day_start} and {target_datetime}"
                    )
                    return pd.DataFrame()
                ...
                df = self._add_multi_image_metadata(df, day_start, target_datetime)

                logger.info(
                    "[DataExtractor] Selected best-quality image for today: "
                    f"room_id={room_id}, image_path={df.iloc[0].get('image_path')}, "
                    f"quality={df.iloc[0].get('image_quality_score')}"
                )
            ...
            logger.error(
                f"[DataExtractor] Failed to extract best-quality embedding: {e}",
                exc_info=True,
            )
            return pd.DataFrame()
    ...
    def extract_historical_embedding_data_for_similarity(
        ...
        try:
            ...
            date_start = None
            if analysis_datetime is not None and date_window_days > 0:
                date_start = analysis_datetime - timedelta(days=date_window_days)
            ...
            with Session(self.db_engine) as session:
                ...
                if date_start is not None:
                    query = query.where(
                        and_(
                            MushroomImageEmbedding.collection_datetime >= date_start,
                            MushroomImageEmbedding.collection_datetime
                            <= analysis_datetime,
                        )
                    )
                ...
                logger.debug(
                    f"[DataExtractor] Top historical batches: {dict(batch_counts)}"
                )
    ...
    def _extract_env_parameters_from_sensor_data(
            ...
            for idx, row in df.iterrows():
                if row["env_sensor_status"] is not None:
                    try:
                        ...
                        logger.debug(
                            f"[DataExtractor] Failed to parse sensor data for row {idx}: {e}"
                        )
                        continue
    ...
    def find_top_similar_historical_cases(
        ...
        try:
            ...
            if historical_df.empty:
                logger.warning(
                    "[DataExtractor] No historical data available for similarity matching"
                )
                return pd.DataFrame()
                ...
                try:
                    if row["embedding"] is not None:
                        ...
                        if hist_embedding.shape == current_embedding.shape:
                            ...
                            logger.debug(
                                f"[DataExtractor] Embedding shape mismatch for row {idx}"
                            )
                    else:
                        logger.debug(f"[DataExtractor] No embedding data for row {idx}")
                except Exception as e:
                    logger.debug(
                        f"[DataExtractor] Error calculating similarity for row {idx}: {e}"
                    )
                    continue

            if not similarities:
                logger.warning(
                    "[DataExtractor] No valid embeddings found for similarity calculation"
                )
                return pd.DataFrame()
            ...
            for idx, row in valid_df.iterrows():
                temp_diff = (
                    abs(row["temperature"] - current_temp)
                    if (current_temp is not None and row["temperature"] is not None)
                    else float("inf")
                )

                humidity_diff = (
                    abs(row["humidity"] - current_humidity)
                    if (current_humidity is not None and row["humidity"] is not None)
                    else float("inf")
                )

                co2_diff = (
                    abs(row["co2"] - current_co2)
                    if (current_co2 is not None and row["co2"] is not None)
                    else float("inf")
                )
    ...
    def _add_multi_image_metadata(
        self, df: pd.DataFrame, aggregation_start: datetime, aggregation_end: datetime
    ) -> pd.DataFrame:
        """
        ...
        df["time_weight"] = (
            1.0 - (time_diffs / max_time_diff) * 0.5
        )  # Weight range: 0.5 to 1.0
    ...
    def extract_realtime_env_data(
        self, room_id: str, target_datetime: datetime
    ) -> pd.DataFrame:
        """
        使用实时历史数据接口提取当天的环境数据
        ...
        try:
            ...
            logger.debug(f"[DataExtractor] 时间范围: [{start_time}, {end_time}]")
            ...
            try:
                ...
                if "point_alias" in all_query_df.columns:
                    alias_mask = all_query_df["point_alias"].isin(
                        required_point_aliases
                    )
            ...
            try:
                ...
                for device_alias in unique_device_aliases:
                    # 获取该设备的配置
                    device_config_df = all_query_df[
                        all_query_df["device_alias"] == device_alias
                    ].copy()

                    try:
                        # 为该设备查询历史数据
                        device_data = query_data_by_batch_time(
                            device_config_df, start_time, end_time
                        )
                        ...
                        if not device_data.empty:
                            ...
                            logger.debug(f"[DataExtractor] 设备 {device_alias} 无数据")
                ...
                # 合并所有设备的数据
                if all_data_frames:
                    df = pd.concat(all_data_frames, ignore_index=True).sort_values(
                        "time"
                    )
                else:
                    df = pd.DataFrame()
                ...
                logger.debug(f"[DataExtractor] 成功获取 {len(df)} 条实时环境数据记录")

            except Exception as e:
                logger.error(f"[DataExtractor] 实时数据查询失败: {e}", exc_info=True)
                return pd.DataFrame()
            ...
            try:
                ...
                if df_room.empty:
                    logger.warning(f"[DataExtractor] 未找到库房 {room_id} 的数据记录")
                    return pd.DataFrame()

                df_room = df_room[
                    (df_room["time"] >= start_time) & (df_room["time"] < end_time)
                ].copy()
                if df_room.empty:
                    logger.warning(f"[DataExtractor] 时间范围内无库房 {room_id} 的数据")
                    return pd.DataFrame()
                ...
                # 按统计日期和测点名称分组计算统计指标
                agg = (
                    df_points.groupby(["stat_date", "point_name"])["value"]
                    .agg(
                        count="count",
                        min="min",
                        max="max",
                ...
                if agg.empty:
                    logger.warning(f"[DataExtractor] 聚合结果为空: room_id={room_id}")
                    return pd.DataFrame()
                ...
                def _col(name):
                    return (
                        ...
                        else pd.Series([np.nan] * len(df_flat))
                    )

                # 温度统计
                df_rec["temp_count"] = (
                    pd.to_numeric(_col("temperature_count"), errors="coerce")
                    .fillna(0)
                    .astype(int)
                )
                ...
                # 湿度统计
                df_rec["humidity_count"] = (
                    pd.to_numeric(_col("humidity_count"), errors="coerce")
                    .fillna(0)
                    .astype(int)
                )
                ...
                # CO2统计
                df_rec["co2_count"] = (
                    pd.to_numeric(_col("co2_count"), errors="coerce")
                    .fillna(0)
                    .astype(int)
                )
                ...
                m_info = df_room[
                    df_room["device_name"]
                    .astype(str)
                    .str.startswith(f"mushroom_info_{room_id}")
                ]
                if not m_info.empty:
                    ...
                    if not m_info_pivot.empty:
                        ...
                        if keep_cols:
                            ...
                            if "in_day_num" in df_rec.columns:
                                ...
                                df_rec["is_growth_phase"] = (
                                    (df_rec["in_day_num"] >= 1)
                                    & (df_rec["in_day_num"] <= 27)
                                ).astype(bool)
                            else:
                ...
                logger.error(
                    f"[DataExtractor] 环境数据统计处理失败: {e}", exc_info=True
                )
                return pd.DataFrame()

        except Exception as e:
            logger.error(f"[DataExtractor] 提取实时环境数据失败: {e}", exc_info=True)
            return pd.DataFrame()
    ...
    def _get_device_configs_cached_for_realtime(self, room_id: str) -> Optional[Dict]:
        ...
        # 检查缓存是否有效
        if (
            hasattr(self, "_realtime_device_config_cache")
            and cache_key in self._realtime_device_config_cache
            and hasattr(self, "_realtime_cache_timestamp")
            and cache_key in self._realtime_cache_timestamp
            and (
                current_time - self._realtime_cache_timestamp[cache_key]
            ).total_seconds()
            < 300
        ):  # 5分钟缓存
    ...
    def extract_env_daily_stats(
        self, room_id: str, target_date: date, days_range: int = 1
    ) -> pd.DataFrame:
        """
        ...
        try:
            ...
            logger.debug(f"[DataExtractor] Date range: [{min_date}, {max_date}]")
            ...
            with Session(self.db_engine) as session:
                ...
                query = (
                    ...
                    .order_by(
                        MushroomEnvDailyStats.stat_date.asc()
                    )  # Ascending order for trend calculation
                )
            ...
            logger.error(
                f"[DataExtractor] Failed to extract env stats: {e}", exc_info=True
            )
            return pd.DataFrame()
    ...
    def _compute_env_trends(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(1, len(df)):
            ...
            if pd.notna(prev_row["temp_median"]) and pd.notna(curr_row["temp_median"]):
                if prev_row["temp_median"] != 0:
                    temp_change = (
                        (curr_row["temp_median"] - prev_row["temp_median"])
                        / prev_row["temp_median"]
                        * 100
                    )
            ...
            # Humidity trend
            if pd.notna(prev_row["humidity_median"]) and pd.notna(
                curr_row["humidity_median"]
            ):
                if prev_row["humidity_median"] != 0:
                    humidity_change = (
                        (curr_row["humidity_median"] - prev_row["humidity_median"])
                        / prev_row["humidity_median"]
                        * 100
                    )
            ...
            if pd.notna(prev_row["co2_median"]) and pd.notna(curr_row["co2_median"]):
                if prev_row["co2_median"] != 0:
                    co2_change = (
                        (curr_row["co2_median"] - prev_row["co2_median"])
                        / prev_row["co2_median"]
                        * 100
                    )
        ...
        logger.debug(f"[DataExtractor] Computed trends for {len(df)} records")
    ...
    def extract_device_changes(
        ...
        try:
            ...
            with Session(self.db_engine) as session:
                ...
                if device_types:
                    conditions.append(
                        DeviceSetpointChange.device_type.in_(device_types)
                    )
                ...
                query = (
                    ...
                    .order_by(
                        DeviceSetpointChange.change_time.desc()
            ...
            logger.error(
                f"[DataExtractor] Failed to extract device changes: {e}", exc_info=True
            )
            return pd.DataFrame()
