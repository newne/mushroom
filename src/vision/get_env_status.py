...
def parse_iot_dataframe_to_records(
    ...
    for room_id in rooms:
        ...
        humidifier_count = (hu_df["l_status"] != 3).astype(int) + (
            hu_df["r_status"] != 3
        ).astype(int)
        ...
        records = [
            {
                ...
                "image_path": image_paths.get(
                    (idx, room_id),
                    f"/default/{room_id}_{ct.strftime('%Y%m%d_%H%M%S')}.jpg",
                ),
                "in_date": date(i["in_year"], i["in_month"], i["in_day"])
                if i["in_year"] and i["in_month"] and i["in_day"]
                else date(1970, 1, 1),
                "in_num": i["in_num"],
                ...
                "semantic_description": _build_semantic_desc(
                    ...
                    light_count=lc,
                    light_on=lt["on_mset"],
                    light_off=lt["off_mset"],
                    left_status=hu["l_status"],
                    left_on=hu["l_on"],
                    left_off=hu["l_off"],
                    right_status=hu["r_status"],
                    right_on=hu["r_on"],
                    right_off=hu["r_off"],
                ),
            }
...
if __name__ == "__main__":
    ...
    df1 = df.pivot_table(
        index="time", columns=["room", "device_name", "point_name"], values="value"
    )
