...
class TemplateRenderer:
    ...
    def _escape_json_examples(self, content: str) -> str:
        ...
        content = re.sub(
            json_block_pattern, escape_json_block, content, flags=re.DOTALL
        )
    ...
    def render(
        ...
        try:
            ...
            template_vars.update(
                {
                    "device_status_section": self._generate_device_status_section(
                        current_data
                    ),
    ...
    def _map_current_environment(self, current_data: Dict) -> Dict:
        ...
        return {
            ...
            "temp_warning": current_temp + 2
            if isinstance(current_temp, (int, float))
            else "数据缺失",
            "humidity_warning": current_humidity - 5
            if isinstance(current_humidity, (int, float))
            else "数据缺失",
            "co2_warning": current_co2 + 300
            if isinstance(current_co2, (int, float))
            else "数据缺失",
        }
    ...
    def _map_device_configs(self, current_data: Dict) -> Dict:
        ...
        formatted_air_cooler = self._format_device_config(
            air_cooler_config, "air_cooler"
        )
        ...
        variables.update(
            {
                ...
                "air_cooler_temp_set": self._get_fallback_value(
                    formatted_air_cooler.get("temp_set"), 15.0
                ),
                "air_cooler_temp_diff": self._get_fallback_value(
                    formatted_air_cooler.get("temp_diffset"), 2.0
                ),
                "air_cooler_cyc_mode": formatted_air_cooler.get(
                    "cyc_on_off", "数据缺失"
                ),
            }
        )
        ...
        formatted_fresh_fan = self._format_device_config(
            fresh_fan_config, "fresh_air_fan"
        )
        variables.update(
            {
                "fresh_air_mode": formatted_fresh_fan.get("mode", "数据缺失"),
                "fresh_air_control": formatted_fresh_fan.get("control", "数据缺失"),
                "fresh_air_co2_on": self._get_fallback_value(
                    formatted_fresh_fan.get("co2_on"), 1000
                ),
                "fresh_air_co2_off": self._get_fallback_value(
                    formatted_fresh_fan.get("co2_off"), 800
                ),
                "fresh_air_time_on": self._get_fallback_value(
                    formatted_fresh_fan.get("on"), 10
                ),
                "fresh_air_time_off": self._get_fallback_value(
                    formatted_fresh_fan.get("off"), 10
                ),
            }
        )
        ...
        formatted_humidifier = self._format_device_config(
            humidifier_config, "humidifier"
        )
        variables.update(
            {
                "humidifier_mode": formatted_humidifier.get("mode", "数据缺失"),
                "humidifier_on": self._get_fallback_value(
                    formatted_humidifier.get("on"), 85
                ),
                "humidifier_off": self._get_fallback_value(
                    formatted_humidifier.get("off"), 90
                ),
            }
        )
        ...
        variables.update(
            {
                ...
                "grow_light_on_time": self._get_fallback_value(
                    formatted_light.get("on_mset"), 60
                ),
                "grow_light_off_time": self._get_fallback_value(
                    formatted_light.get("off_mset"), 60
                ),
    ...
    def _format_historical_data(
        self, env_stats: pd.DataFrame, device_changes: pd.DataFrame
    ) -> str:
        ...
        if not device_changes.empty:
            ...
            for device_type in device_changes["device_type"].unique():
                ...
                for _, row in device_df.head(3).iterrows():  # Show top 3 changes
                    ...
                    lines.append(
                        f"  * {change_time}: {point_name} {prev_val} → {curr_val}"
                    )
    ...
    def render_enhanced(
        ...
        try:
            ...
            if multi_image_analysis:
                template_vars.update(
                    self._map_multi_image_context(multi_image_analysis)
                )
            else:
    ...
    def _map_multi_image_context(
        self, multi_image_analysis: "MultiImageAnalysis"
    ) -> Dict:
        """
        ...
        consistency_info = (
            f"图像一致性: {consistency_level} (分数: {consistency_score:.2f})"
        )

        # Format view consistency information
        view_consistency_map = {"high": "高", "medium": "中", "low": "低"}
        view_consistency_cn = view_consistency_map.get(
            multi_image_analysis.view_consistency, "未知"
        )
        ...
        for device_id, device_info in self.monitoring_points_config.items():
            ...
            for point in point_list:
                ...
                if change_type == "enum_state":
                    ...
                    if enum_mapping:
                        options = "/".join(
                            [f"{k}={v}" for k, v in enum_mapping.items()]
                        )
