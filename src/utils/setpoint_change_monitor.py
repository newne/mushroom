...
class DeviceSetpointChangeMonitor:
    ...
    def __init__(self, config_manager: Optional[SetpointConfigManager] = None):
        ...
        logger.info(
            f"Initialized setpoint monitor with {len(self.setpoint_configs)} configurations from static settings"
        )
    ...
    def _initialize_setpoint_configs_from_static(self) -> List[SetpointConfig]:
        ...
        try:
            ...
            for device_type_key in datapoint_config.keys():
                ...
                try:
                    ...
                    if not hasattr(device_type_config, "point_list"):
                        logger.debug(
                            f"Device type {device_type_key} has no point_list, skipping"
                        )
                        continue
                    ...
                    if device_type_key in device_types:
                        monitored_points = self.config_manager.get_monitored_points(
                            device_type_key
                        )
                        ...
                        for point in point_list:
                            ...
                            if not point_name or not point_alias:
                                logger.warning(
                                    f"Invalid point configuration in {device_type_key}: missing point_name or point_alias"
                                )
                                continue
                            ...
                            if point_alias in monitored_points:
                                ...
                                description = point.get(
                                    "description", f"{device_type_key}.{point_alias}"
                                )
                                ...
                                logger.debug(
                                    f"Added setpoint config: {device_type_key}.{point_name} -> {point_alias} ({change_type.value})"
                                )
                    else:
                        logger.debug(
                            f"No setpoint definitions found for device type: {device_type_key}"
                        )
            ...
            logger.info(
                f"Successfully loaded {len(configs)} setpoint configurations from static settings"
            )

            # 按设备类型分组显示加载的配置
            device_type_counts = {}
            for config in configs:
                device_type_counts[config.device_type] = (
                    device_type_counts.get(config.device_type, 0) + 1
                )
            ...
            logger.error(
                f"Failed to initialize setpoint configs from static settings: {e}"
            )
            # 如果从静态配置加载失败，返回空列表
            return []

    def _determine_change_type(
        self, point_config: Dict[str, Any], threshold: Optional[float]
    ) -> ChangeType:
        """
        根据测点配置和阈值确定变更类型
        ...
        if data_type in ["bool", "boolean"] or "on_off" in point_config.get(
            "point_alias", ""
        ):
    ...
    def get_room_setpoint_data(
        ...
        try:
            ...
            setpoint_df = all_query_df[
                all_query_df["point_alias"].isin(setpoint_aliases)
            ].copy()
            ...
            logger.info(
                f"Querying setpoint data for room {room_id}, time range: {start_time} ~ {end_time}"
            )
            ...
            def get_device_type(row):
                ...
                fallback_type = config_mapping.get(point_alias, {}).get(
                    "device_type", "unknown"
                )
                ...
                if fallback_type != "unknown":
                    logger.debug(
                        f"Using fallback device type mapping: {device_alias or device_name}.{point_alias} -> {fallback_type}"
                    )

                return fallback_type

            df["device_type"] = df.apply(get_device_type, axis=1)
            df["change_type"] = df["point_name"].map(
                lambda x: config_mapping.get(x, {}).get("change_type", "unknown")
            )
            df["threshold"] = df["point_name"].map(
                lambda x: config_mapping.get(x, {}).get("threshold")
            )
            df["description"] = df["point_name"].map(
                lambda x: config_mapping.get(x, {}).get("description", "")
            )
    ...
    def detect_setpoint_changes(
        self, setpoint_data: pd.DataFrame
    ) -> List[Dict[str, Any]]:
        """
        检测设定点变更
        ...
        try:
            ...
            logger.debug(
                f"Processing {len(grouped_data)} device-point combinations for change detection"
            )
            ...
            for (device_name, point_name), group in grouped_data:
                ...
                if len(group) < 2:
                    logger.debug(
                        f"Skipping {device_name}.{point_name}: insufficient data points ({len(group)})"
                    )
                ...
                try:
                    ...
                    logger.warning(
                        f"Missing configuration field for {device_name}.{point_name}: {e}"
                    )
                    continue
                ...
                for i in range(1, len(group)):
                    ...
                    if change_type == ChangeType.DIGITAL_ON_OFF.value:
                        ...
                        if int(current_value) != int(previous_value):
                            ...
                            change_info = (
                                f"{int(previous_value)} -> {int(current_value)}"
                            )
                        ...
                        if (
                            threshold
                            and abs(current_value - previous_value) >= threshold
                        ):
                            change_detected = True
                    ...
                    if change_detected:
                        ...
                        logger.debug(
                            f"Change detected: {device_name}.{point_name} - {change_info}"
                        )

                if group_changes > 0:
                    logger.debug(
                        f"Found {group_changes} changes for {device_name}.{point_name}"
                    )

            logger.info(
                f"Processed {processed_groups} device-point combinations, detected {len(changes)} setpoint changes"
            )
            return changes
    ...
    def monitor_room_setpoint_changes(
        self, room_id: str, hours_back: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.info(
                f"Monitoring setpoint changes for room {room_id}, time range: {start_time} ~ {end_time}"
            )
    ...
    def monitor_all_rooms_setpoint_changes(
        ...
        try:
            ...
            for room_id in rooms:
                try:
                    ...
                    if changes:
                        logger.info(
                            f"Room {room_id}: found {len(changes)} setpoint changes"
                        )
                    else:
            ...
            logger.info(
                f"Monitoring completed: {successful_rooms}/{len(rooms)} rooms processed successfully"
            )
            logger.info(
                f"Total setpoint changes detected across all rooms: {total_changes}"
            )
    ...
    def store_setpoint_changes(self, changes: List[Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            if missing_fields:
                raise ValueError(
                    f"Missing required fields in change records: {missing_fields}"
                )
...
def batch_monitor_setpoint_changes(
    ...
    if time_diff.days > max_days:
        logger.warning(
            f"Large time range detected: {time_diff.days} days (max recommended: {max_days}). This may take a long time to process."
        )
    ...
    try:
        ...
        for i, room_id in enumerate(rooms, 1):
            try:
                ...
                # 获取库房设定点数据
                setpoint_data = monitor.get_room_setpoint_data(
                ...
                if changes:
                    logger.info(
                        f"Room {room_id}: Detected {len(changes)} setpoint changes"
                    )
        ...
        if store_results and all_changes:
            ...
            try:
                ...
                if success:
                    ...
                    logger.info(
                        f"✅ Successfully stored {len(all_changes)} change records to database"
                    )
                else:
...
def validate_batch_monitoring_environment(
    config_manager: Optional[SetpointConfigManager] = None,
) -> bool:
    """
    验证批量监控环境的可用性 (重构版本)
    ...
    try:
        ...
        try:
            ...
            if not monitor.setpoint_configs:
                ...
                return False
            logger.debug(
                f"✅ Monitor created with {len(monitor.setpoint_configs)} configurations"
            )
        except Exception as e:
...
if __name__ == "__main__":
    ...
    if changes:
        ...
        for i, change in enumerate(changes[:3], 1):  # 显示前3个
            ...
            prev_value = change.get("previous_value")
            curr_value = change.get("current_value")
            print(f"      变更: {prev_value} -> {curr_value}")
