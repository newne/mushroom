...
class OutputHandler:
    ...
    def _create_error_enhanced_output(
        ...
        # Create empty/error device recommendations structure
        # Since we might not have config here if initialization failed or for other reasons,
        # we return an empty structure or minimal structure.
        ...
        return EnhancedDecisionOutput(
            ...
            monitoring_points=MonitoringPoints(),
            multi_image_analysis=MultiImageAnalysis(0, []),
            metadata=DecisionMetadata(errors=errors)
        )
