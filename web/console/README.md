...
当前位置、目标站位、距离和当前动作是可信读数：控制器实读优先，其次是巡检站位推定、最后一次
成功目标，无法确认时明确显示未知，不把未知位置当成 0 mm。灯状态也只接受执行方返回的
`result.data.lamp_on`，失败的命令不会乐观改写按钮。
...
回归脚本：`docs/patrol/console-ui/verify-page.js`（jsdom，**91 项断言**）。
