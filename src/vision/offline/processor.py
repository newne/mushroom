...
def _offline_processor_log(event: str, message: str, level: str = "INFO", **context):
    log_task_event(
        "VISION_OFFLINE_PROCESSOR",
        event,
        message,
        level=level,
        task_type="helper",
...
class OfflineProcessor:
    ...
    def _load_prompt_from_registry(
        self, prompt_name="growth_stage_describe", version="3"
    ):
        """Loads prompt from MLflow Registry"""
        try:
            mlflow.set_tracking_uri(config.mlflow_tracking_uri)
            _offline_processor_log(
                "VISION_OFFLINE_PROCESSOR_PROMPT_LOAD_START",
                "开始从 MLflow Registry 加载 prompt",
            ...
            _offline_processor_log(
                "VISION_OFFLINE_PROCESSOR_PROMPT_LOAD_FAILED",
                "从 MLflow Registry 加载 prompt 失败，回退默认 prompt",
                level="WARNING",
                prompt_name=prompt_name,
                version=version,
                status="failed",
    ...
    def process_daily_batch(
        ...
        """
        _offline_processor_log(
            "VISION_OFFLINE_PROCESSOR_BATCH_START",
            "开始执行离线批量处理",
        ...
        try:
            ...
            _offline_processor_log(
                "VISION_OFFLINE_PROCESSOR_METADATA_FAILED",
                "获取离线批量处理元数据失败",
                level="ERROR",
                status="failed",
                error_type=type(e).__name__,
        ...
        for item in items:
            ...
            _offline_processor_log(
                "VISION_OFFLINE_PROCESSOR_ITEM_START",
                "开始处理离线图像",
            ...
            try:
                ...
                self.mlflow_logger.log_analysis_result(
                    ...
                    chinese_desc=description_result.get("chinese_description", "")
                    or "N/A",
                    english_desc=description_result.get("growth_stage_description", "")
                    or "N/A",
                    metadata={
                        "room_id": room_id,
                        "collection_datetime": str(collection_datetime),
                ...
                _offline_processor_log(
                    "VISION_OFFLINE_PROCESSOR_ITEM_FAILED",
                    "离线图像处理失败",
                    level="ERROR",
                    image_path=image_path,
                    room_id=room_id,
        ...
        _offline_processor_log(
            "VISION_OFFLINE_PROCESSOR_BATCH_FINISH",
            "离线批量处理完成",
            total_items=self.total_processed + self.failed_count,
            successful_items=self.total_processed,
            failed_items=self.failed_count,
            status="success" if self.failed_count == 0 else "partial",
        )
    ...
    def _generate_description(self, image: Image.Image) -> Dict[str, Any]:
            ...
            prompt_messages = [
                {
                    ...
                    or "Describe the mushroom growth stage in JSON format.",
                },
                {
                    "role": "user",
                    "content": "Please analyze the image and generate the JSON output.",
                },
            ]
        ...
        for msg in prompt_messages:
            ...
            if role == "system":
                ...
                openai_messages.append(
                    {"role": str(role or "user"), "content": str(content)}
                )
                if role == "user":
                    last_user_index = len(openai_messages) - 1
        ...
        base_url_template = getattr(
            self.llama_config, "llama_completions", "http://{0}:{1}/v1/chat/completions"
        )
        ...
        for i in range(max_retries):
            try:
                response = requests.post(
                    ...
                    json=payload,
                    headers=headers,
                    timeout=getattr(self.llama_config, "timeout", 60),
                )
                ...
                _offline_processor_log(
                    "VISION_OFFLINE_PROCESSOR_LLAMA_RETRY",
                    "LLaMA API 调用失败，准备重试",
                    level="WARNING",
                    attempt=i + 1,
    ...
    def _parse_llama_content(self, content: str) -> Dict[str, Any]:
        ...
        _offline_processor_log(
            "VISION_OFFLINE_PROCESSOR_LLAMA_INVALID_JSON",
            "LLaMA 响应不是有效 JSON，回退原始文本结构",
            level="WARNING",
            status="failed",
        )
        return {
