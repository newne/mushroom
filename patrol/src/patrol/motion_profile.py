...
| --- | --- | --- | --- | --- |
| Y（轴 1） | 0 … 4492 | 向右 | 左端 | 负限位回零（homeDir=2） |
| Z（轴 2） | -212 … 0 | 向上 | 顶端 | 正限位回零（homeDir=1） |

原点即「Y 反向（负限位）回零、Z 向上（正限位）回零」后的落点。两轴行程都从原点
**单侧**展开，于是坐标符号自带方向语义：Y 增大 = 向右，Z 减小 = 向下。这个不变式由
...
HOME_DIR_POSITIVE = 1   # 正限位回零（向运动正方向找开关）
HOME_DIR_NEGATIVE = 2   # 负限位回零（向运动负方向找开关）
...
# 推导与对照表见 `.scratch/prod-deploy/speed-tune.py`（可重跑）。
#
...
_Y = AxisProfile(
    ...
    home_dir=HOME_DIR_NEGATIVE,   # 反向（负限位）回零
    home_speed=90.0,
    home_acc=900.0,
    home_release=5.0,
...
_Z = AxisProfile(
    ...
    travel_speed=20.0,
    travel_acc=200.0,
    home_dir=HOME_DIR_POSITIVE,   # 向上（正限位）回零
    home_speed=20.0,
    home_acc=200.0,
    home_release=5.0,
)
