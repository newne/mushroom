...
def _vision_log(event: str, message: str, level: str = "INFO", **context: Any) -> None:
    """输出统一的视觉任务事件日志。"""
    log_task_event("CLIP_INFERENCE", event, message, level=level, **context)
...
class CLIPInferenceTask(BaseTask):
    ...
    def execute_task(self) -> Dict[str, Any]:
        ...
        _vision_log(
            ...
            lookback_hours=self.hour_lookback,
        )
        ...
        # 创建编码器
        _vision_log("CLIP_ENCODER_INIT", "初始化蘑菇图像编码器", status="running")
        ...
        _vision_log(
            ...
            "图像处理批次完成",
            status="success",
            total_items=total_stats["total"],
            successful_items=total_stats["success"],
            failed_items=total_stats["failed"],
            skipped_items=total_stats["skipped"],
            success_rate=round(success_rate, 2),
        )

        # 记录各库房处理情况
        for room_id in self.rooms:
            stats = room_stats.get(
                room_id, {"total": 0, "success": 0, "failed": 0, "skipped": 0}
            )
            _vision_log(
                "CLIP_ROOM_SUMMARY",
                "库房处理统计",
        ...
        try:
            ...
            if processing_stats:
                _vision_log(
                    "CLIP_PROCESSING_STATS",
                    "已获取图像处理统计",
                    status="success",
                    stored_records=processing_stats.get("total_processed", 0),
        ...
        if (
            total_stats["total"] > 0
            and (total_stats["failed"] / total_stats["total"]) > 0.1
        ):
            _vision_log(
                "CLIP_FAILURE_RATE_HIGH",
                "图像处理失败率偏高",
                level="WARNING",
                status="partial",
                total_items=total_stats["total"],
            ...
            with pgsql_engine.connect() as conn:
                ...
                for room_id in self.rooms:
                    ...
                    if row:
                        summary["rooms_summary"][room_id] = {
                            ...
                            "embedding_rate": (row[1] / row[0]) * 100
                            if row[0] > 0
                            else 0,
                        }
                ...
                if row:
                    summary["overall_stats"] = {
                        ...
                        "overall_embedding_rate": (row[2] / row[0]) * 100
                        if row[0] > 0
                        else 0,
                    }
            ...
            with pgsql_engine.connect() as conn:
                ...
                if row:
                    ...
                    validation_result = {
                        ...
                        "embedding_completeness": row[1] / total_records
                        if total_records > 0
                        else 0,
                        "quality_distribution": {
                            "high_quality": row[2],
                            "low_quality": row[3],
                            "high_quality_rate": row[2] / total_records
                            if total_records > 0
                            else 0,
                            "low_quality_rate": row[3] / total_records
                            if total_records > 0
                            else 0,
                        },
                        "quality_stats": {
                    ...
                    quality_score = (
                        1
                        - validation_result["quality_distribution"]["low_quality_rate"]
                    ) * 50
                    validation_result["overall_quality_score"] = (
                        embedding_score + quality_score
                    )
...
def safe_hourly_clip_inference() -> None:
    ...
    if not result.get("success", False):
        ...
        _vision_log(
            ...
            status="success",
            task_run_id=result.get("task_run_id"),
            total_items=result.get("total_stats", {}).get("total", 0),
            successful_items=result.get("total_stats", {}).get("success", 0),
            failed_items=result.get("total_stats", {}).get("failed", 0),
            skipped_items=result.get("total_stats", {}).get("skipped", 0),
