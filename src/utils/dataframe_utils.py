...
def _is_cache_valid(redis_key: str, config_file_path: Path) -> bool:
    ...
    try:
        ...
        if cache_timestamp is None:
            logger.debug(
                f"Cannot get cache timestamp for {redis_key}, assuming cache invalid"
            )
            return False
...
def _set_cache_with_metadata(
    redis_key: str, data: str, ttl: int = REDIS_CACHE_TTL
) -> bool:
    """
    设置缓存并记录元数据
...
def get_static_config_by_device_type(device_type: str) -> pd.DataFrame:
    ...
    try:
        ...
        device_query_df = (
            ...
            .merge(
                device_df.drop(columns=["remark"], errors="ignore").assign(
                    __cart_key=1
                ),
                on="__cart_key",
            )
        ...
        # 写入 Redis（带TTL和元数据）
        df_json = device_query_df.to_json(
            orient="records", force_ascii=False, indent=None
        )
...
def get_all_device_configs(room_id: str = None) -> Dict[str, pd.DataFrame]:
    ...
    try:
        ...
        device_types = [
            key
            for key, value in datapoint_config.items()
            ...
            try:
                ...
                if room_id is not None:
                    ...
                    if room_devices:
                        ...
                        # 回退方案：使用设备名称后缀过滤
                        filtered_df = df[
                            df["device_name"].str.endswith(f"_{room_id}", na=False)
                        ].copy()
                        if not filtered_df.empty:
                            all_configs[device_type] = filtered_df
                            success_count += 1
...
def clear_device_config_cache(device_type: str = None) -> bool:
    ...
    try:
        if device_type:
            # 清除指定设备类型的缓存
            device_key = mushroom_redis_key["static_config"].format(
            ...
            logger.info(
                f"Cleared cache for device type '{device_type}', deleted {deleted_count} keys"
            )
            return True
        else:
            # 清除所有设备类型的缓存
            try:
                ...
                logger.info(
                    f"Cleared cache for all device types, deleted {total_deleted} keys"
                )
                return True
            ...
            try:
                ...
                all_info["_summary"] = {
                    ...
                    "cached_types": sum(
                        ...
                        if isinstance(info, dict) and info.get("cache_exists", False)
                    ),
                    "valid_caches": sum(
                        1
                        for info in all_info.values()
                        if isinstance(info, dict) and info.get("cache_valid", False)
                    ),
