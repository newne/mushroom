...
def format_dynamic_result_summary(result) -> str:
    ...
    if result.apply_time:
        summary_lines.append(f"⚡ Applied: {result.apply_time.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    try:
        ...
        for i, config in enumerate(results, 1):
            if args.verbose:
                ...
                print(f"{i:3d}. {config.room_id} | {config.device_type:15s} | {config.device_alias:20s} | {config.point_alias:15s} | {config.remark or 'N/A'}")
            
            # 收集统计信息
            device_types[config.device_type] = device_types.get(config.device_type, 0) + 1
    ...
    try:
        ...
        for i, result in enumerate(results, 1):
            if args.verbose:
                ...
                change_icon = "🔄" if result.change else "➖"
                status_icon = {"pending": "⏳", "applied": "✅", "rejected": "❌", "failed": "💥"}.get(result.status, "❓")
                time_str = result.time.strftime('%m-%d %H:%M') if result.time else 'N/A'
                print(f"{i:3d}. {result.room_id} | {time_str} | {result.device_type:12s} | {result.point_alias:12s} | {change_icon} {result.old}→{result.new} | {status_icon} {result.status}")
            ...
            device_stats[result.device_type] = device_stats.get(result.device_type, 0) + 1
        
        # 显示统计摘要
        print("\n" + "=" * 80)
        print("📊 SUMMARY STATISTICS:")
        print(f"   Total Results: {len(results)}")
        print(f"   Changes: {change_stats['changes']}/{change_stats['total']} ({change_stats['changes']/change_stats['total']*100:.1f}%)")
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description="Query decision analysis static configs and dynamic results")
    ...
    parser.add_argument("--point-alias", type=str, help="Point alias filter (dynamic only)")
    parser.add_argument("--changes-only", action="store_true", 
                       help="Show only records with changes (dynamic only)")
    parser.add_argument("--status", type=str, choices=["pending", "applied", "rejected", "failed"],
                       help="Status filter (dynamic only)")
    parser.add_argument("--hours", type=int, help="Show results from last N hours (dynamic only)")
    parser.add_argument("--days", type=int, help="Show results from last N days (dynamic only)")
