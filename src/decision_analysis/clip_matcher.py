...
class CLIPMatcher:
        ...
        try:
            ...
            for _, row in similar_df.iterrows():
                try:
                    ...
                    logger.warning(f"[CLIPMatcher] Error creating SimilarCase object: {e}")
                    continue
            ...
            logger.error(f"[CLIPMatcher] Error finding similar cases: {e}", exc_info=True)
            return []
    ...
    def find_similar_cases_multi_image(
        ...
        """
        logger.info(f"[CLIPMatcher] Finding similar cases with multi-image boost (images: {image_count})")
    ...
    def _apply_multi_image_boost(
        ...
        consistency_boost = 1.0 + (images_in_window / max(total_images, 1)) * 0.1  # Consistency boost
