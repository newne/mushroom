...
class OutputHandler:
    ...
    def validate_and_format(self, raw_decision: Dict, room_id: str) -> DecisionOutput:
        ...
        air_cooler_params = {
            k: v for k, v in air_cooler_params.items() if v is not None
        }
        is_valid, validation_errors = self._validate_device_params(
        ...
        # Filter out None values before validation
        humidifier_params = {
            k: v for k, v in humidifier_params.items() if v is not None
        }
        is_valid, validation_errors = self._validate_device_params(
        ...
        # Filter out None values before validation
        grow_light_params = {
            k: v for k, v in grow_light_params.items() if v is not None
        }
        is_valid, validation_errors = self._validate_device_params(
        ...
        # Create metadata
        metadata = DecisionMetadata(warnings=warnings, errors=errors)
    ...
    def _validate_device_params(
        self, device_type: str, params: Dict
    ) -> Tuple[bool, List[str]]:
        """
        ...
        device_config = (
            self.static_config.get("mushroom", {}).get("datapoint", {}).get(device_type)
        )
        ...
        if device_type == "air_cooler":
            ...
            if "tem_set" in params:
                ...
                if not (5 <= tem_set <= 25):
                    errors.append(
                        f"air_cooler.tem_set: Value {tem_set} out of range [5, 25]"
                    )
            ...
            if "tem_diff_set" in params:
                ...
                if not (0.5 <= tem_diff <= 5):
                    errors.append(
                        f"air_cooler.tem_diff_set: Value {tem_diff} out of range [0.5, 5]"
                    )
            ...
            if "cyc_on_time" in params:
                ...
                if not (1 <= cyc_on <= 60):
                    errors.append(
                        f"air_cooler.cyc_on_time: Value {cyc_on} out of range [1, 60]"
                    )
            ...
            if "cyc_off_time" in params:
                ...
                if not (1 <= cyc_off <= 60):
                    errors.append(
                        f"air_cooler.cyc_off_time: Value {cyc_off} out of range [1, 60]"
                    )
            ...
            if "co2_on" in params:
                ...
                if not (400 <= co2_on <= 5000):
                    errors.append(
                        f"fresh_air_fan.co2_on: Value {co2_on} out of range [400, 5000]"
                    )
            ...
            if "co2_off" in params:
                ...
                if not (400 <= co2_off <= 5000):
                    errors.append(
                        f"fresh_air_fan.co2_off: Value {co2_off} out of range [400, 5000]"
                    )
            ...
            if "on" in params:
                ...
                if not (1 <= on_time <= 120):
                    errors.append(
                        f"fresh_air_fan.on: Value {on_time} out of range [1, 120]"
                    )
            ...
            if "off" in params:
                ...
                if not (1 <= off_time <= 120):
                    errors.append(
                        f"fresh_air_fan.off: Value {off_time} out of range [1, 120]"
                    )
            ...
            if "on" in params:
                ...
                if not (0 <= hum_on <= 100):
                    errors.append(
                        f"humidifier.on: Value {hum_on} out of range [0, 100]"
                    )
            ...
            if "off" in params:
                ...
                if not (0 <= hum_off <= 100):
                    errors.append(
                        f"humidifier.off: Value {hum_off} out of range [0, 100]"
                    )
            ...
            if "on_mset" in params:
                ...
                if not (1 <= on_mset <= 1440):
                    errors.append(
                        f"grow_light.on_mset: Value {on_mset} out of range [1, 1440]"
                    )
            ...
            if "off_mset" in params:
                ...
                if not (1 <= off_mset <= 1440):
                    errors.append(
                        f"grow_light.off_mset: Value {off_mset} out of range [1, 1440]"
                    )
    ...
    def _correct_invalid_params(
        ...
        if device_type == "air_cooler":
            ...
            if "tem_set" in corrected:
                ...
                warnings.append(
                    "air_cooler.tem_set: Missing field, set to default 15.0"
                )
            ...
            if "tem_diff_set" in corrected:
                ...
                warnings.append(
                    "air_cooler.tem_diff_set: Missing field, set to default 2.0"
                )
            ...
            if "cyc_on_time" in corrected:
                ...
                warnings.append(
                    "air_cooler.cyc_on_time: Missing field, set to default 10"
                )
            ...
            if "cyc_off_time" in corrected:
                ...
                warnings.append(
                    "air_cooler.cyc_off_time: Missing field, set to default 10"
                )
            ...
            for field, default in [
                ...
                if field not in corrected:
                    ...
                    warnings.append(
                        f"air_cooler.{field}: Missing field, set to default {default}"
                    )
            ...
            if "co2_on" in corrected:
                ...
                warnings.append(
                    "fresh_air_fan.co2_on: Missing field, set to default 1000"
                )
            ...
            if "co2_off" in corrected:
                ...
                warnings.append(
                    "fresh_air_fan.co2_off: Missing field, set to default 800"
                )
            ...
            for field, default in [("model", 0), ("control", 0)]:
                if field not in corrected:
                    corrected[field] = default
                    warnings.append(
                        f"fresh_air_fan.{field}: Missing field, set to default {default}"
                    )
            ...
            for field, default in [
                ...
                if field not in corrected:
                    corrected[field] = default
                    warnings.append(
                        f"grow_light.{field}: Missing field, set to default {default}"
                    )
    ...
    def validate_and_format_enhanced(
        ...
        # If critical structure is missing, return error status
        if errors:
            return self._create_error_enhanced_output(
                room_id, errors, multi_image_analysis
            )
        ...
        if self.monitoring_points_config:
            ...
            def _iter_monitoring_devices():
                devices_cfg = self.monitoring_points_config
                if isinstance(devices_cfg, dict) and "devices" in devices_cfg:
                    devices_cfg = devices_cfg.get("devices", {})
                if not isinstance(devices_cfg, dict):
                    return []

                items = []
                for device_type, device_list in devices_cfg.items():
                    if isinstance(device_list, list):
                        for device_info in device_list:
                            items.append((device_type, device_info))
                    elif isinstance(device_list, dict) and "point_list" in device_list:
                        items.append((device_type, device_list))
                return items

            # Dynamic validation based on monitoring_points_config
            for device_type, device_info in _iter_monitoring_devices():
                # Skip non-device keys
                if not isinstance(device_info, dict) or "point_list" not in device_info:
                    continue

                device_alias = device_info.get("device_alias", device_type)
                ...
                if not device_params and device_type not in [
                    "humidifier",
                    "grow_light",
                ]:
                ...
                validated_device = self._validate_dynamic_device(
                    device_type, device_params, points, warnings, errors
                )

                # Use alias as key in output map
                devices_output[device_alias] = validated_device
                if device_type and device_type not in devices_output:
                    devices_output[device_type] = validated_device
        else:
        ...
        for point_config in points_config:
            ...
            if enum_mapping:
                ...
                if curr_str not in enum_mapping:
                    ...
                    param_adj.current_value = (
                        int(default_key) if default_key.isdigit() else default_key
                    )
                    warnings.append(
                        f"{device_type}.{point_alias}: Invalid current value '{current_val}', reset to '{default_key}'"
                    )

                # Check recommended value
                rec_str = (
                    str(int(recommended_val))
                    if isinstance(recommended_val, (int, float))
                    else str(recommended_val)
                )
                if rec_str not in enum_mapping:
                    # Try to use default
                    default_key = list(enum_mapping.keys())[0]
                    param_adj.recommended_value = (
                        int(default_key) if default_key.isdigit() else default_key
                    )
                    warnings.append(
                        f"{device_type}.{point_alias}: Invalid recommended value '{recommended_val}', reset to '{default_key}'"
                    )
            ...
            if change_type in ["analog_value"]:
                try:
                    ...
                    warnings.append(
                        f"{device_type}.{point_alias}: Expected float, got {type(param_adj.current_value)}"
                    )
                ...
                try:
                    ...
                    warnings.append(
                        f"{device_type}.{point_alias}: Expected int, got {type(param_adj.current_value)}"
                    )
    ...
    def _extract_parameter_adjustment(
        self, param_data: Dict, param_name: str, device_type: str
    ) -> ParameterAdjustment:
        ...
        if not isinstance(param_data, dict):
            ...
            return ParameterAdjustment(
                ...
                risk_assessment=RiskAssessment(
                    adjustment_risk="low", no_action_risk="low", impact_scope="无影响"
                ),
            )
