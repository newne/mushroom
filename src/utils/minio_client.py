...
def handle_minio_errors(
    operation_name: str, default_return: Any = None, raise_on_error: bool = False
):
    """统一错误处理装饰器"""
    ...
    def decorator(func):
        ...
        def wrapper(self, *args, **kwargs):
            try:
                ...
                self._handle_error(
                    operation_name, e, raise_on_error=raise_on_error, **context
                )
...
class MinIOClient:
    ...
    def _create_client(self, http_client: Optional[PoolManager] = None) -> Minio:
        ...
        try:
            ...
            parsed = (
                urlparse(raw_endpoint)
                if "://" in raw_endpoint
                else urlparse(f"//{raw_endpoint}")
            )
            inferred_secure = (
                parsed.scheme.lower() == "https" if parsed.scheme else None
            )
            ...
            if isinstance(e, ssl.SSLError) or "WRONG_VERSION_NUMBER" in str(e):
                logger.error(
                    "创建MinIO客户端失败（SSL 错误）: 可能是使用了 HTTPS 连接到仅支持 HTTP 的 MinIO 服务。请检查配置 'MINIO.endpoint' 是否包含 scheme (http/https) 或在配置中显式设置 'secure = false'。"
                )
            logger.error(f"创建MinIO客户端失败: {e}")
            raise

    def _handle_error(
        self, operation: str, error: Exception, raise_on_error: bool = True, **context
    ) -> Optional[Any]:
        """统一错误处理函数"""
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"连接测试成功，存储桶 {bucket_name} {'存在' if exists else '不存在'}"
            )
            return True
        except S3Error as e:
            self._handle_error(
                "test_connection", e, raise_on_error=False, bucket=self.config["bucket"]
            )
            return False
        except Exception as e:
        ...
        try:
            if not self.client.bucket_exists(bucket_name):
                self.client.make_bucket(
                    bucket_name, location=self.config.get("region", "us-east-1")
                )
            ...
            self._handle_error(
                "ensure_bucket_exists", e, raise_on_error=False, bucket=bucket_name
            )
            return False
        except Exception as e:
    ...
    def _folder_date_matches_timestamp(
        self, object_name: str, capture_time: datetime
    ) -> bool:
    ...
    def list_rooms(self, bucket_name: Optional[str] = None) -> List[str]:
        ...
        try:
            ...
            self._handle_error(
                "list_rooms", e, raise_on_error=False, bucket=bucket_name
            )
            return []
        except Exception as e:
    ...
    def _build_prefixes_by_room_and_days(
        self, room_id: Optional[str], ymds: List[str], bucket_name: str
    ) -> List[str]:
        """构建基于库房和日期的前缀列表"""
        prefixes = []
    ...
    def list_images_by_time_and_room(
        ...
        try:
            ...
            logger.info(
                f"查询图片 - 库房: {room_id or '全部'}, 时间范围: {start_time} ~ {end_time}, "
                f"前缀数量: {len(prefixes)}"
            )
            ...
            for prefix in prefixes:
                try:
                    ...
                    for obj in objects:
                        ...
                        # 可选的一致性校验
                        if (
                            validate_folder_date
                            and not self._folder_date_matches_timestamp(
                                obj.object_name, capture_time
                            )
                        ):
                            logger.warning(
                                f"文件夹日期与时间戳不一致，跳过: {obj.object_name}"
                            )
                            continue
    ...
    def list_recent_images(
        ...
        logger.info(
            f"查询最近 {hours} 小时的图片 - 库房: {room_id or '全部'}, "
    ...
    def get_image_bytes(
        ...
        try:
            ...
            logger.info(
                f"成功获取图片字节数据: {object_name}, 大小: {len(image_data)} bytes"
            )
            return image_data
    ...
    def upload_image(
        ...
        try:
            ...
            logger.info(
                f"成功上传图片: {file_path} -> {bucket_name}/{object_name} ({content_type})"
            )
            return True
    ...
    def upload_bytes(
        ...
        try:
            ...
            logger.info(
                f"成功上传字节数据: {bucket_name}/{object_name}, 大小: {len(data)} bytes ({content_type})"
            )
            return True
    ...
    # 兼容性接口 - 保持原有方法名和行为
    def list_images(
        self, bucket_name: Optional[str] = None, prefix: str = ""
    ) -> List[str]:
        """
        列出存储桶中的图片文件 - 兼容性接口
...
def create_http_client_with_pool(
    timeout: int = 30, retries: int = 3, pool_connections: int = 10
) -> PoolManager:
    """
    创建带连接池的HTTP客户端示例
